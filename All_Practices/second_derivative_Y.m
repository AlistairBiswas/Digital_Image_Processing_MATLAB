...
% Second forward derivative along the Y-axis
for i = 1:row-2
    for j = 1:col
        x = S(i+2, j) - 2 * S(i+1, j) + S(i, j);
        R2_forward(i, j) = x;
    end
end

% Second backward derivative along the Y-axis
for i = 3:row
    for j = 1:col
        x  = S(i, j) - 2 * S(i-1, j) + S(i-2, j);
        R2_backward(i, j) = x;
    end
end

% Second central derivative along the Y-axis
for i = 2:row-1
    for j = 1:col
        x = S(i+1, j) - 2 * S(i, j) + S(i-1, j);
        R2_central(i, j) = x;
    end
end
